...
export function cancellationClientEmailHTML(d: BookingData): string {
  return `<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #F5F1E8; padding: 40px 20px; margin: 0;">
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width: 560px; margin: 0 auto;">
    <tr><td style="padding: 0 8px;">
      ...
      <p style="color: #8A8470; font-size: 13px; margin: 0 0 32px; font-style: italic; letter-spacing: 0.5px;">El arte de cuidarte</p>
      <h2 style="font-size: 22px; margin: 0 0 10px; color: #1C1A14; font-weight: 600;">Hola ${d.nombre},</h2>
      <p style="color: #5C5840; line-height: 1.6; font-size: 15px; margin: 0 0 8px;">Tu turno fue cancelado:</p>
    </td></tr>
    <tr><td style="padding: 8px;">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background: #FEFDF8; border-radius: 16px; border: 1px solid rgba(0,0,0,.06);">
        <tr><td style="padding: 8px 28px;">
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
            ${row("📅 &nbsp; Fecha y hora", `${d.fecha} — ${d.horario} hs`)}
            ${row("💆 &nbsp; Servicio", d.servicio, true)}
          </table>
        </td></tr>
      </table>
    </td></tr>
    <tr><td style="padding: 16px 8px 0;">
      <p style="color: #5C5840; line-height: 1.6; font-size: 14px; margin: 0 0 16px;">Disculpá las molestias. Cuando quieras reagendar, podés escribirnos por WhatsApp o reservar un nuevo turno desde la web.</p>
      <p style="color: #5C5840; line-height: 1.6; font-size: 14px; margin: 0;">¡Te esperamos pronto! 💚</p>
      <hr style="border: none; border-top: 1px solid rgba(0,0,0,.08); margin: 36px 0 16px;"/>
      <p style="color: #8A8470; font-size: 11px; margin: 0; letter-spacing: 0.3px;">GF Studio — Hipólito Yrigoyen 229, Henderson</p>
    </td></tr>
  </table>
</div>
</body>
